...
New version of autowave, based on July 2008 autoslice.c by Earl Kirkland
Incorporates 11-27-08 version of WriteDMFixedFormat and
code from the old 2001 version of autowave
...
int main()
{
	...
	/*Variables for wave output per slice calculations: PMV*/
	FILE *th_f;
	char th_fname[255], th_fname_base[255];
	int amp, phase, amp_rot;
	
	float th_x_min, th_x_max, th_y_min, th_y_max;
	int th_nx, th_ny, th_nx_off, th_ny_off;
	float **th_dat;
	int th_ix, th_iy, th_wx, th_wy;

	int th_nr, th_ri, th_px, th_py;
	float th_rx, th_ry;
	float **th_r_im;
	double *th_r_dat, *th_r_pix;
...
/*  get simulation options */
	...
	/* remove intensity vs depth cross section part of Earl's code */
	lcross = 0;
	...
	scanf("%f", &ycross );
      	...
      	/*Get user input for wave function v thckness.  PMV*/
	amp = askYN("Do you want to record the amplitude at each slice");
	phase = askYN("Do you want to record the phase at each slice");
	amp_rot = askYN("Do you want to record the enclosed amplitude versus radius at each slice?");
	if(!amp && !phase && !amp_rot) {
		printf("You should record something at each slice, or use a different program.\n");
		exit(0);
	}

	if(amp || phase) {
	  printf("Please enter the position and size of the\n");
	  printf("wave output image (xi, xf, yi, yf) in A:\n");
	  scanf("%f %f %f %f", &th_x_min, &th_x_max, &th_y_min, &th_y_max);
	}

	if(amp_rot) {
	  printf("Please enter the location about which to do the annular average in A:\n");
	  scanf("%f %f", &th_rx, &th_ry);
	}
	
	printf("Please enter the base filename for the wave evolution results:\n");
	scanf("%s", th_fname_base);
...
/* 	/\* remove Earl's intensity cross-section code *\/ */
/* 	/\* */
/*         if( lcross == 1 ) { */
...
/* set up the image output arrays, PMV */

	if(amp || phase) {
	  th_nx = (int) ceil( (th_x_max - th_x_min)/ax * nx );
	  th_ny = (int) ceil( (th_y_max - th_y_min)/by * ny );
	  th_dat = (float**) malloc2D(th_nx, th_ny, sizeof(float), "wave function output image");
	  
	  th_nx_off = (int) floor( th_x_min/ax * nx );
	  th_ny_off = (int) floor( th_y_min/by * ny );
	  
	  printf("\n");
	  printf("The output wave function image is %d x %d pixels.\n", th_nx, th_ny);
	  printf("It starts at pixel (%d, %d) in the calculated wave function.\n", 
		 th_nx_off, th_ny_off);
	}

	if(amp_rot) {
	  th_px = (int) ceil( th_rx / ax * nx );
	  th_py = (int) ceil( th_ry / by * ny );
	  th_nr = min( min( th_px, (nx-th_px) ), min( th_py, (ny - th_py) ) );
	  th_r_dat = (double*) malloc1D(th_nr, sizeof(double), "Annular integration data array");
	  th_r_im = (float**) malloc2D(th_nr, 1, sizeof(float), "Annular integration output image");

	  printf("\n");
	  printf("The annular average will be about pixel (%d, %d),\n", th_px, th_py);
	  printf("and will cover a radius of %d pixels.\n", th_nr);
	} /* end if(amprot) */
...
/* ---- start coherent method below ----------------
		    ...
		    /* calculate the wave function magnitude and dump it to an image file PMV */
		    /*printf("Start thick calc this slice.\n");*/
			
		    if(amp) {
		      sprintf(th_fname, "%s_%.3d_amp.gfx", th_fname_base, islice);
		      if(!(th_f = fopen(th_fname, "wb")) ) {
			printf("Cannot open the output image.  Exiting.\n");
			exit(0);
		      }
		      for(th_ix = 0; th_ix < th_nx; th_ix++) {
			for(th_iy = 0; th_iy < th_ny; th_iy++) {
			  th_wx = th_ix + th_nx_off;
			  th_wy = th_iy + th_ny_off;
			  th_dat[th_ix][th_iy] = waver[th_wx][th_wy]*waver[th_wx][th_wy] +
			    wavei[th_wx][th_wy]*wavei[th_wx][th_wy];
			  th_dat[th_ix][th_iy] *= scale;
			  
			}
		      }
		      
		      WriteDMFixedFormat(th_f, th_dat, (long)th_nx, (long)th_ny);
		      fclose(th_f);	
		    }
		    
		    if(phase) {
		      sprintf(th_fname, "%s_%.3d_phase.gfx", th_fname_base, islice);
		      if(!(th_f = fopen(th_fname, "wb")) ) {
			printf("Cannot open the output image.  Exiting.\n");
			exit(0);
		      }
			...
			for(th_iy = 0; th_iy < th_ny; th_iy++) {
			  th_wx = th_ix + th_nx_off;
			  th_wy = th_iy + th_ny_off;
			  th_dat[th_ix][th_iy] = (float)atan(wavei[th_wx][th_wy]/waver[th_wx][th_wy]);
		    ...
		    if(amp_rot) {
		      sprintf(th_fname, "%s_%.3d_ampenc.gfx", th_fname_base, islice);
		      if(!(th_f = fopen(th_fname, "wb")) ) {
			printf("Cannot open the output image.  Exiting.\n");
			exit(0);
		      }
		      for(th_ri = 0; th_ri < th_nr; th_ri++)
			th_r_dat[th_ri] = 0.0;
		      
		      for(th_ix = 0; th_ix < nx; th_ix++) {
			for(th_iy = 0; th_iy < ny; th_iy++) {
			  th_ri = sqrt( (th_ix-th_px)*(th_ix-th_px) + (th_iy - th_py)*(th_iy-th_py) );
			  if(th_ri < th_nr) {
			    th_r_dat[th_ri] += (waver[th_ix][th_iy]*waver[th_ix][th_iy] +
						wavei[th_ix][th_iy]*wavei[th_ix][th_iy]);
			  }
			}
		      }
		      ...
		      sum = th_r_dat[0];
		      for(th_ri = 1; th_ri < th_nr; th_ri++) {
			sum += th_r_dat[th_ri];
			th_r_dat[th_ri] += th_r_dat[th_ri-1];
			th_r_dat[th_ri-1] *= scale;			   
		      }
		      th_r_dat[th_nr-1] *= scale;
		      sum *= scale;

		      for(th_ri=0; th_ri<th_nr; th_ri++)
			th_r_im[th_ri][0] = (float) th_r_dat[th_ri];
		      WriteDMFixedFormat(th_f, th_r_im, (long)th_nr, 1l);
		      fclose(th_f);
		      printf("Annular average sum = %f\n", sum);
		      printf("Annular average image final pixel = %f\n", th_r_im[th_nr-1][0]);
		      printf("Annular average double-precision data block final pixel: = %f\n", th_r_dat[th_nr-1]);
		    }
		    
		    /*printf("Finish thick calc this slice.\n");*/
...
/* write an array to a Gatan Fixed Format (.gfx) file.  pmv */
