...
! add new module scattering_factors, add USE without :: following them
! for
...
program rmc
    ...
    ! recenter_model is called in read_model so it doesnt need to be
    ! called
    ...
    ! Fortran's max int is 2147483647, and 2147483647 + 1 = -2147483648,
    ! so this
    ! loop will stop at i= one after i=2147483647.
    ! The loops also stops if the temperature goes below a certain temp
    ! (30.0).
    do while (i >0)
        i=i+1

        call random_move(m,w,xx_cur,yy_cur,zz_cur,xx_new,yy_new,zz_new, max_move)
        ! check_curoffs returns false if the new atom placement is too
        ! close to
        ! another atom. Returns true if the move is okay. (hard shere
        ! cutoff)
        do while( .not. check_cutoffs(m,cutoff_r,w) )
            ! Check_cutoffs returned false so reset positions and try
            ! again.
            m%xx(w) = xx_cur
            m%yy(w) = yy_cur
            m%zz(w) = zz_cur
        ...
        ! TODO Jason - Check if this is being done as well as it could
        ! be.
        ! the accept and reject functions shouldn't both have the same
        ! do loops
        ! in them.
        write(*,*) "Accepting or rejecting move."
        randnum = ran2(iseed2)
        ! Test if the move should be accepted or rejected based on
        ! del_chi
        if(del_chi <0.0)then
            ! Accept the move
            e1 = e2
            ...
            ! Based on the random number above, even if del_chi is
            ! negative, decide
        ...
        ! Periodically save data. Are we saving the energy here??? We
        ! should!
        ! The energy will tell us if there is a steady decline, or if it
        ! statistically jumped way up. We could also write del_chi (or
        ! instead).
