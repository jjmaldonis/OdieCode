...
subroutine scatt_power(m,used_data_sets,istat)
	...
	if(used_data_sets(1)) then   
		...
		!open(unit=901,file="test_scat_fact_e.txt",form='formatted',status='unknown')
		!do j=1,m%nelements
		!	do k=1, 100
		!		q=k*scat_binsize
		!		write(901,*)j,q,scatfact_e(j,k)  
 		!	enddo
		!enddo
		!close(901)
...
subroutine gr_initialize(m,r_e,gr_e,r_n,gr_n,r_x,gr_x,used_data_sets,istat)
	...
	!WRITE(*,*) 'radius: ', radius, mbin_e*del_r_e !debug
...
subroutine gr_hutch_mc(m,w,xx_cur,yy_cur,zz_cur,xx_new,yy_new,zz_new,used_data_sets,istat)   
	...
	real vb, nid, rho
	real xi, yi, zi, xj, yj, zj, xij, yij, zij, R, R2
	real, INTENT(IN) :: xx_cur,yy_cur,zz_cur , xx_new,yy_new,zz_new    !current and new  positions of atoms, by Feng Yi on 03/03/2009
	...
	!Test printing 
	!open(unit=906,file="test_gr_part_cur.txt",form='formatted',status='unknown')
