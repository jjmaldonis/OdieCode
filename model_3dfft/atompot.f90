! Data, functions, and subroutines for evaluating the electrostatic potential
! of a model, using atomic potentials from Earl Kirkland.  Requires the module
! recipes.f90, which in turn requires nrtypes.f90 and nrutils.f90.
!
! subroutines:
!
! vatom(integer Z, real radius)
!   returns the electrostatic potential for a neutral atom of atomic number Z
!   at a distance radius away, in Angstroms.  Translated from slicelib.c by
!   Earl Kirkland
!
! modelpot_3D(model m, integers npixx, npixy, npixz, complex 3D array pointer modelpot)
!   allocates, then fills in modelpot with the potentials of all the atoms in 
!   model m, as complex numbers.  Complex for future use in in-place Fourier transforms.
!   Might eventually be useful to write an optional-arguments version of this that could
!   take either a real or complex array pointer.  This routine can use a LOT of memory:
!      (1024, 1024, 1024) reals is 4.0 GB (1 GB of 4-bytes each)
!      (2048, 2048, 2048) reals is 32 GB
!      (512, 512, 512) complexes is 1.0 GB
!      (1024, 1024, 1024) complexes is 8 GB.  
!   
! Changelog
!   first version, 01-27-09, pmv
!   changed model_pot from complex to real
...
module atompot
  ...
  private
  public :: vatom, model_pot3D, int_pot, int_model_pot3D

  ! parameters and variables required by vatom
  integer, parameter :: NZMIN = 1, NZMAX = 103, NPMAX = 12
  integer, parameter :: nl = 3, ng = 3
  real, save, pointer, dimension(:,:) :: fparams
  logical, save :: feTableRead = .FALSE.

  ! variables for numerical recipes 3D integration functions
  REAL(SP) :: xsav,ysav,x1,x2,y1,y2,z1,z2,xatom,yatom,zatom
  integer :: Znum


  real, parameter :: POT_RMAX = 0.25
  integer, parameter :: NPOT = 500
  ...
  type potLUT
     ...
     real, dimension(:), pointer :: rpos
  end type potLUT

  type(potLUT), dimension(NZMAX), save :: pLUT
  
  contains

    ! calculates the integrated potential over the volume centered on each pixel.
    ! by calling vatom.  Still needs to precalculate the V(r) LUT.
    subroutine int_model_pot3D(m, npixx, npixy, npixz, modelpot)
      type(model), intent(in) :: m
      integer, intent(in) :: npixx, npixy, npixz
      real, pointer, dimension(:,:,:), intent(out) :: modelpot

      real :: xmin, xmax, ymin, ymax, zmin, zmax
      real :: stepx, stepy, stepz
      integer :: iatom, jatom, katom, istat
      integer :: n, i, j, k, ii, jj, kk, irad, jrad, krad
      real :: px, py, pz, r

      allocate(modelpot(npixx, npixy, npixz), stat=istat)
      if( istat /= 0) then
         write (*,*) 'Allocation of model potential array failed. Exiting.'
         stop
      endif
      modelpot = cmplx(0.0, 0.0)
      
      write (*,*)
      write (*,*) '   Calculating atomic potentials.'
      do i=1,m%nelements
         call vatomLUT_init(m%atom_type(i))
      enddo
      
      stepx = m%lx / real(npixx)
      stepy = m%ly / real(npixy)
      stepz = m%lz / real(npixz)
      
      irad = ceiling(POT_RMAX / stepx)
      jrad = ceiling(POT_RMAX / stepy)
      krad = ceiling(POT_RMAX / stepz)

      write (*,*) '   Each atom occupies:'
      write (*,*) '   ',irad,' pixels in x.'
      write (*,*) '   ',jrad,' pixels in y.'
      write (*,*) '   ',krad,' pixels in z.'
      write (*,*) '   Calculating the 3D model potential.'

      ! add the potential from each atom to the model_pot array
      do n=1, m%natoms
         if (mod(n, 50) == 0) then
            write (*,*) 'Adding potential for atom ', n
         endif
         
         ! loop over the block in model_pot centered on (iatom, jatom, katom)
         katom = anint((m%zz(n)+m%lz/2.0) / stepz)
         do k=(katom-krad), (katom+krad)
            pz = -0.5*m%lz + real(k)*stepz
            zmin = pz - stepz/2.0
            zmax = pz + stepz/2.0
            pz = (pz - m%zz(n))**2
            kk = k
            if (k < 1) kk = k+npixz      ! periodic boundary conditions
            if (k > npixz) kk = k-npixz  ! periodic boundary conditions
            
            jatom = anint((m%yy(n)+m%ly/2.0) / stepy)
            do j = (jatom-jrad), (jatom+jrad)
               py = -0.5*m%ly + real(j)*stepy
               ymin = py - stepy/2.0
               ymax = py + stepy/2.0
               py = (py - m%yy(n))**2
               jj = j
               if (j < 1) jj = j+npixy
               if (j > npixy) jj = j-npixy
               
               iatom = anint((m%xx(n)+m%lx/2.0) / stepx)
               do i = (iatom-irad), (iatom+irad)
                  px = -0.5*m%lx + real(i)*stepx
                  xmin = px - stepx/2.0
                  xmax = px + stepx/2.0
                  px = (px - m%xx(n))**2
                  ii = i
                  if (i < 1) ii = i+npixx
                  if (i > npixx) ii = i-npixx

                  r = sqrt( px + py +pz )
                  if (r <= POT_RMAX ) then
                     modelpot(ii,jj,kk) = modelpot(ii,jj,kk) + int_pot(m%znum(n), m%xx(n), &
                          m%yy(n), m%zz(n), xmin, xmax, ymin, ymax, zmin, zmax)
                  endif

               enddo ! i / x in model_pot
            enddo  ! j / y in model_pot
         enddo  ! k / z in model_pot
      enddo  ! n in atoms list

    end subroutine int_model_pot3D


    ! calculates the integrated potential in Volts-Angstroms^3 in the rectangular prism
    ! (xmin to xmax), (ymin to ymax), (zmin to zmax) from an atom with atomic number Z
    ! at a position (xa, ya, za)
    function int_pot(Z, xa, ya, za, xmin, xmax, ymin, ymax, zmin, zmax)
      real :: int_pot
      integer, intent(in) :: Z
      ...
      call quad3D_qgaus(int_pot)

    end function int_pot


    ! calculates the electrostatic potential of the atoms in the model m and places it
    ! in a (npixx, npiyx, npixz) array of complex numbers in modelpot.
    subroutine model_pot3D(m, npixx, npixy, npixz, modelpot)
      type(model), intent(in) :: m
      integer, intent(in) :: npixx, npixy, npixz
      real, pointer, dimension(:,:,:), intent(out) :: modelpot
      ...
      write (*,*)
      write (*,*) '3D model potential step size is:'
      write (*,*) stepx,' Angstroms in x'
      write (*,*) stepy,' Angstroms in y'
      write (*,*) stepz,' Angstroms in z'
      write (*,*)
      ...
      do n=1, m%natoms
         ...
         katom = anint((m%zz(n)+m%lz/2.0) / stepz)
         do k=(katom-krad), (katom+krad)
            pz = (-0.5*m%lz + real(k)*stepz - m%zz(n))**2
            kk = k
            if (k < 1) kk = k+npixz      ! periodic boundary conditions
            if (k > npixz) kk = k-npixz  ! periodic boundary conditions
            ...
            jatom = anint((m%yy(n)+m%ly/2.0) / stepy)
            do j = (jatom-jrad), (jatom+jrad)
               py = (-0.5*m%ly + real(j)*stepy - m%yy(n))**2
               jj = j
               if (j < 1) jj = j+npixy
               if (j > npixy) jj = j-npixy
               
               iatom = anint((m%xx(n)+m%lx/2.0) / stepx)
               do i = (iatom-jrad), (iatom+jrad)
                  px = (-0.5*m%lx + real(i)*stepx - m%xx(n))**2
                  ii = i
                  if (i < 1) ii = i+npixx
                  if (i > npixx) ii = i-npixx
                  ...
                  r = sqrt( px + py +pz )
                  if (r <= POT_RMAX ) then 
                     modelpot(ii,jj,kk) = modelpot(ii,jj,kk) + vatomLUT(m%znum(n), r)
                  endif
    ...
    !
    !  assumed module parameters and variables:
    !
    !     NZMIN   1   = min Z in featom.tab 
    !     NZMAX   103 = max Z in featom.tab 
    !
    !     logical feTableRead  flag to remember if the param file has been read 
    !     integer nl=3, ng=3   number of Lorenzians and Gaussians 
    !     real fparams[][] = fe parameters
    !
    ...
    !  stolen from ejk and translated from C to Fortran  01-25-09, pmv
    
    function vatom(Z, radius)
      real vatom
      integer, intent(in) :: Z
      real, intent(in) :: radius
      
      integer :: i
      real :: suml, sumg, x, t, r
      
      ! Lorenzian, Gaussian constants
      real, parameter :: al=150.4121417, ag=266.5985798
      real, parameter :: pi=3.141592654
      
      if( (Z<NZMIN) .OR. (Z>NZMAX) ) return
      
      ! read in the table from a file if this is the
      ! first time this is called
      if( .NOT. feTableRead ) then
         call read_fparams()
      endif
      ...
      x = 2.0*pi*r
      do i=0, 2*nl, 2
         suml = suml + fparams(Z, i)* exp( -x*sqrt(fparams(z,i+1) ) )
      enddo
      
      ! Gaussians
      x = pi*r
      x = x*x
      do i=2*nl, 2*(nl+ng)-1, 2
         t = sqrt( fparams(Z, i+1) )
         t = t*t*t
         sumg = sumg + fparams(Z, i)*exp(-x/fparams(Z, i+1)) / t
      enddo
      
      vatom = al*suml/r + ag*sumg
      
    end function vatom
    

    function vatomLUT(Z, radius)
      real :: vatomLUT
      integer :: Z
      real :: radius

      integer :: rp_min, rp_max
      real :: r, pot_d

      if( .NOT. pLUT(Z)%calculated ) call vatomLUT_init(Z)

      r = abs(radius)

      if( r > POT_RMAX ) then
!         write (*,*) 'vatomLUT called outside its range, which is slow.'
         vatomLUT = vatom(Z, r)
         return
      endif

      rp_min = floor(r/pLUT(Z)%rstep)-1
      if(rp_min < 1) rp_min = 1
      rp_max = rp_min + 2
      if(rp_max > pLUT(Z)%npts) rp_max = pLUT(Z)%npts
      call polint(pLUT(Z)%rpos(rp_min:rp_max), pLUT(Z)%pot(rp_min:rp_max), r, vatomLUT, pot_d)

    end function vatomLUT

    subroutine vatomLUT_init(Z)
      integer :: Z

      integer :: i

      pLUT(Z)%rstep = POT_RMAX / NPOT
      pLUT(Z)%npts = NPOT

      allocate(pLUT(Z)%pot(NPOT), pLUT(Z)%rpos(NPOT))
      do i=1, NPOT
         pLUT(Z)%rpos(i) = pLUT(Z)%rstep*real(i)
         pLUT(Z)%pot(i) = vatom(Z, pLUT(Z)%rpos(i))
      enddo
      pLUT(Z)%calculated = .TRUE.
   
    end subroutine vatomLUT_init


!! functions for quad3D integration from numerical recipes
    FUNCTION h(x)
    ...
    FUNCTION func(x,y,z)
      ...
      do i=1,size(z)
         ...
         z2 = z2*z2
         r = sqrt(x2 + y2 + z2)
         func(i) = vatomLUT(Znum, r)
      enddo

    END FUNCTION func

    !  fill in scattering factor parameter data from fparams.dat by ejk
    subroutine read_fparams()
      
      allocate(fparams(NZMAX, 0:NPMAX-1))
