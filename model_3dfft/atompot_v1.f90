...
module atompot
  ...
  private
  public :: vatom, model_pot3D, int_model_pot3D
  ...
  contains
    ...
    subroutine int_model_pot3D(m, npixx, npixy, npixz, modelpot)
      ...
      complex, pointer, dimension(:,:,:), intent(out) :: modelpot
    ...
    subroutine model_pot3D(m, npixx, npixy, npixz, modelpot)
      ...
      real, parameter :: POT_CUT = 1.0e-6
      real :: stepx, stepy, stepz
      integer :: iatom, jatom, katom, cut_p_max, zi, istat
      integer :: n, a, i, j, k, ii, jj, kk, rpolint_min
      real :: px, py, pz, r, pot0, rmax, rpot_step, pot, pot_d
      real, dimension(:,:), pointer :: rpot
      real, dimension(:), pointer :: rcut, rpos
      integer, dimension(:,:), pointer :: cut_p
      ...
      allocate(rcut(m%nelements), cut_p(m%nelements, 3), stat = istat)
      do i=1,m%nelements
         pot0 = vatom(m%atom_type(i), 1.0e-6)
         j = 2
         do
            if ( vatom(m%atom_type(i), 1.0e-6*real(j)) / pot0 <= POT_CUT ) then
               rcut(i) = 1.0e-6*real(j)
               exit
            endif
            j = j + 1
         enddo
         write (*,*) 'Potential cutoff for atomic number ',m%atom_type(i),' is ',rcut(i),' Angstroms.'
      enddo
      
      cut_p_max = 0
      rmax = 0
      do i=1,m%nelements
         cut_p(i,1) = ceiling(rcut(i) / stepx)
         if (cut_p(i,1) > cut_p_max) cut_p_max = cut_p(i,1)
         cut_p(i,2) = ceiling(rcut(i) / stepy)
         if (cut_p(i,2) > cut_p_max) cut_p_max = cut_p(i,2)
         cut_p(i,3) = ceiling(rcut(i) / stepz)
         if (cut_p(i,3) > cut_p_max) cut_p_max = cut_p(i,3)
         
         if (rcut(i) > rmax) rmax = rcut(i)
      enddo
      cut_p_max = cut_p_max*100


      allocate(rpot(cut_p_max, m%nelements), rpos(cut_p_max), stat=istat)
      
      write (*,*) 'Atom potential sampled at ',cut_p_max,' points.'
      
      rpot_step = rmax/real(cut_p_max-1)
      rpos = (/ (rpot_step*real(i), i=0, cut_p_max-1) /)
      do i=1, m%nelements
         do j=1, cut_p_max
            rpot(j, i) = vatom(m%atom_type(i), rpos(j))
         enddo
      enddo
      ...
      do n=1, m%natoms
         ...
         ! figure out which position in rpot corresponds to this Z
         do a=1, m%nelements
            if(m%znum(n) == m%atom_type(a)) then
               zi = a
               exit
            endif
         enddo
         !write (*,*) 'Working on an atom with atomic number ', zi
         
         ! loop over the block in model_pot centered on (iatom, jatom, katom)
         katom = anint((m%zz(n)+m%lz/2.0) / stepz)
         do k=(katom-cut_p(zi, 3)), (katom+cut_p(zi, 3))
            pz = (-0.5*m%lx + real(k)*stepz - m%zz(n))**2
            kk = k
            ...
            do j = (jatom-cut_p(zi,2)), (jatom+cut_p(zi,2))
               ...
               do i = (iatom-cut_p(zi,1)), (iatom+cut_p(zi,1))
                  px = (-0.5*m%lz + real(i)*stepx - m%xx(n))**2
                  ii = i
                  if (i < 1) ii = i+npixx
                  if (i > npixx) ii = i-npixx
                  r = sqrt( px + py +pz )
                  if (r <= rcut(zi) ) then
                     rpolint_min = anint(r/rpot_step) - 1
                     if(rpolint_min <= 1) rpolint_min = 1
                     if(rpolint_min >= (cut_p_max - 2) ) rpolint_min = cut_p_max-2
                     call polint(rpos(rpolint_min:rpolint_min+2), &
                          rpot(rpolint_min:rpolint_min+2, zi), r, pot, pot_d)
                     modelpot(ii,jj,kk) = modelpot(ii,jj,kk) + cmplx(pot, 0.0)
                  endif
